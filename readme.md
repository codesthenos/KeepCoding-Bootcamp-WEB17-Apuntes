# Apuntes Bootcamp WEB17 Keepcoding 2024

#### CONTENIDO DE LAS SUBCARPETAS CLASE 1, 2 ...

Subcarpetas con los apuntes y los ejercicios de clase, la clase 1 contiene la clase 1, la clase 2 empezo siendo una copia de la clase 1 y le añado la clase 2, la clase 3 es la copia de la clase 2 y le añado la clase 3 y asi sucesivamente

## 01-WEB101

- Clase impartida por _Alberto Casero_ [**@kasappeal**](https://github.com/kasappeal) el dia 02/09/2024
- Documento .txt con los apuntes cogidos en clase

## 02-INTRODUCCION-JAVASCRIPT

- Clases impartidas por _Kevin Julián Martínez Escobar_ [**@kevinccbsg**](https://github.com/kevinccbsg)

- Fechas de las clases:

  1. **04/09/2024**

  2. **05/09/2024**
  3. **09/09/2024**
  4. **10/09/2024**
  5. **12/09/2024**
  6. **16/09/2024**

- Fecha entrega de practica: **22/09/2024**
- [Repo de la practica](https://github.com/codesthenos/KC-WEB17-IntroJS-codesthenos)

- _Instrucciones_ para visualizar los diferentes documentos.js:

  1. Cambiar el atributo 'src' del script que hay en la linea 9 del index.html

  2. Si usas visual studio code, con alguna extension como live server puedes abrirlo haciendo click derecho en el archivo y seleccionando la opcion abrir con live server

- Un par de ejemplos:

  1. `<script src="ejercicios/ejercicio8.js"></script>`

  2. `<script src="examples/09_destructuring.js"></script>`

## 03-FUNDAMENTOS-HTML-CSS

- Clases impartidas por _Kevin Julián Martínez Escobar_ [**@kevinccbsg**](https://github.com/kevinccbsg)

- Fechas de las clases:

  1. **23/09/2024**

  2. **24/09/2024**
  3. **26/09/2024**
  4. **30/09/2024**
  5. **01/10/2024**
  6. **03/10/2024**

- Fecha entrega de practica: **13/10/2024**
- [Repo de la practica](https://github.com/codesthenos/KC-WEB17-HTML-CSS-codesthenos)
- [Web de la practica](https://codesthenos.github.io/KC-WEB17-HTML-CSS-codesthenos/)

## 04-MODELADO-DE-DATOS-INTRODUCCION-SQL

- Clases impartidas por _Francisco José Molina Martínez_ [**@franciscomoma**](https://github.com/franciscomoma)

- Fechas de las clases:

  1. **14/10/2024**

  2. **15/10/2024**
  3. **16/10/2024**

- Fecha entrega de practica: **20/10/2024**
- [Repo de la practica](https://github.com/codesthenos/KC-WEB17-SQL-codesthenos)

## 05-DESARROLLO-BACKEND-NODEJS

- Clases impartidas por _Javier Miguel_ [**@jamg44**](https://github.com/jamg44)

- Fechas de las clases:

  1. **21/10/2024**

  2. **22/10/2024**
  3. **24/10/2024**
  4. **28/10/2024**
  5. **29/10/2024**
  6. **31/10/2024**

- Fecha entrega de practica: **10/11/2024**
- [Repo de la practica](https://github.com/codesthenos/KC-WEB17-Desarrollo-backend-NODE-codesthenos-)

## 06-DESARROLLO-FRONTEND-JAVASCRIPT

- Clases impartidas por _Eduardo Aguilar Tirado_ [**@edu-aguilar**](https://github.com/edu-aguilar)

- Fechas de las clases:

  1. **11/11/2024**

  2. **12/11/2024**
  3. **14/11/2024**
  4. **18/11/2024**
  5. **19/11/2024**
  6. **21/11/2024**

- Fecha entrega de practica: **01/12/2024**
- [Repo de la practica](https://github.com/codesthenos/KC-WEB17-Desarrollo-frontend-JavaScript-codesthenos)

## 07-FRONTEND-PRO

- Clases impartidas por _Nauel Gómez_ [**@Nauel G**](https://github.com/NauelG)

- Fechas de las clases:

  1. **02/12/2024**

  2. **03/12/2024**
  3. **05/12/2024**

## 08-WEB-COMPONENTS

- Clases impartidas por _Eduardo Aguilar Tirado_ [**@edu-aguilar**](https://github.com/edu-aguilar)

- Fechas de las clases:

  1. **09/12/2024**

  2. **10/12/2024**
  3. **12/12/2024**

## 09-DESARROLLO-BACKEND-AVANZADO-NODEJS

- Clases impartidas por _Javier Miguel_ [**@jamg44**](https://github.com/jamg44)

- Fechas de las clases:

  1. **16/12/2024**

  2. **17/12/2024**
  3. **19/12/2024**
  4. **07/01/2025**
  5. **08/01/2025**
  6. **09/01/2025**

- Fecha entrega de practica: **19/01/2025**
- [Repo de la practica](https://github.com/codesthenos/KC-WEB17-Backend-Avanzado-NODE-codesthenos)

## 10-TDD-JAVASCRIPT

- Clases impartidas por _Nauel Gómez_ [**@Nauel G**](https://github.com/NauelG)

- Fechas de las clases:

  1. **20/01/2025**

  2. **21/01/2025**
  3. **23/01/2025**

## 11-FUNDAMENTOS-REACT

- Clases impartidas por _David Jimenez_ [**@davidjj76**](https://github.com/davidjj76)

- Fechas de las clases:

  1. **27/01/2025**

  2. **28/01/2025**
  3. **30/01/2025**
  4. **03/02/2025**
  5. **04/02/2025**
  6. **06/02/2025**

- Fecha entrega de practica: **16/02/2025**
- [Repo de la practica](https://github.com/codesthenos/KC-WEB17-Fundamentos-REACT-codesthenos)

## 12-REACT-AVANZADO

- Clases impartidas por _David Jimenez_ [**@davidjj76**](https://github.com/davidjj76)

- Fechas de las clases:

  1. **17/02/2025**

  2. **18/02/2025**
  3. **20/02/2025**
  4. **24/02/2025**
  5. **25/02/2025**
  6. **27/02/2025**

- Fecha entrega de practica: **09/03/2025**
- **Practica no entregada**

## 13-CONFIGURACION-SERVIDORES-DESPLIEGUE-APLICACIONES

- Clases impartidas por _Alberto Casero_ [**@kasappeal**](https://github.com/kasappeal)

- Fechas de las clases:

  1. **10/03/2025**

  2. **11/03/2025**
  3. **13/03/2025**
  4. **17/03/2025**
  5. **18/03/2025**
  6. **20/03/2025**

- Fecha entrega de practica: **30/03/2025**
- [Repo de la practica]()
